...
def process_data():
	...
	# 绑定IP地址和端口号
	server_socket.bind(('192.168.40.80', 12345))
	# 开始监听
	...
	instances = {'192.168.40.81': client1_instance, '192.168.40.82': client2_instance}
	...
	for e in range(conf["global_epochs"]):
		...
		data = {'192.168.40.81': server.global_model.encrypt_weights, '192.168.40.82': server.global_model.encrypt_weights}
		...
		# 更新全局模型
		if '192.168.40.81' in updated_data1:
			diff = updated_data1['192.168.40.81']
			for i in range(len(weight_accumulator)):
				weight_accumulator[i] = weight_accumulator[i] + diff[i]
				# data['192.168.177.11'] = updated_data1['192.168.177.11']

		if '192.168.40.82' in updated_data2:
			diff = updated_data2['192.168.40.82']
			for i in range(len(weight_accumulator)):
				weight_accumulator[i] = weight_accumulator[i] + diff[i]
				# data['192.168.177.12'] = updated_data2['192.168.177.12']
		# print(f"更新后的数据: {data}\n")
